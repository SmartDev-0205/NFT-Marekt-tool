...
def attribute_count_floors_def(params):
    file_name = "attribute_count_floors.py"
    ...
    result_str += "=================================================================\n"
    result_str += f"{sys.argv[1]} attribute count floors on MagicEden"

    for k, v in sorted(floor.items()):
        result_str += f"Attribute count: {k}, floor price: {v[0]} SOL\n"
        result_str += f"    -https://magiceden.io/item-details/{v[1]}\n"
    result_str += "=================================================================\n"

    if len(errors):
        result_str = f"Error with mint addresses: " + errors
    return result_str
