...
def wallet_evaluation_def(params):
    result_str = ""
    file_name = "wallet_evaluation.py"
    ...
    result_str += "\n==========================Wallet Summary=========================="
    result_str += collections[0] + "\n"
    sum = 0
    for mint in wallet_map.keys():
        price = get_price(wallet_map[mint][1], attr_floor_map)
        result_str += "\t" + wallet_map[mint][0] + ": " + str(price) + " SOL\n"
        sum += price
    result_str += "Total Attribute Floor Value: " + str(sum) + " SOL\n"
    return result_str